...
/snap/bin/microk8s kubectl exec -n postgres-prod $POD -- psql -U postgres -c "CREATE USER site15 WITH PASSWORD 'password'"
/snap/bin/microk8s kubectl exec -n postgres-prod -ti $POD -- psql -U postgres -c "CREATE DATABASE site15"
/snap/bin/microk8s kubectl exec -n postgres-prod -ti $POD -- psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE \"site15\" TO \"site15\""
